    ...
    suspend operator fun invoke(item: DownloadItem) {
        ...
        if (item.isManga) {
            val request = OneTimeWorkRequestBuilder<ChapterDownloadWorker>()
                .setInputData(
                    workDataOf(
                        ChapterDownloadWorker.KEY_DOWNLOAD_ID to item.id,
                        ChapterDownloadWorker.KEY_CHAPTER_URL to item.episodePageUrl,
                        ChapterDownloadWorker.KEY_MANGA_TITLE to item.animeTitle,
                        ChapterDownloadWorker.KEY_SOURCE_NAME to item.sourceName,
                        ChapterDownloadWorker.KEY_CHAPTER_TITLE to item.episodeTitle,
                    )
                )
                .addTag(ChapterDownloadWorker.buildTag(item.id))
                .build()
            workManager.enqueueUniqueWork(
                ChapterDownloadWorker.buildTag(item.id),
